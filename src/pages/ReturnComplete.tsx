...
import { CheckCircle, Home as HomeIcon, FileText, RotateCcw } from 'lucide-react'

export function ReturnComplete() {
  ...
  const creditNumber = `CR-${Date.now().toString().slice(-6)}`
  const returnDate = new Date().toLocaleString('en-US', {
    dateStyle: 'medium',
    timeStyle: 'short',
  })
  ...
  const totalItems = cart.reduce((sum: number, item: any) => sum + Math.abs(item.quantity), 0)
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-amber-100 flex flex-col">
      ...
      <div className="bg-white/80 backdrop-blur-md shadow-lg border-b border-amber-200/50 p-6">
        <div className="flex items-center justify-center gap-4">
          ...
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Return Complete</h1>
        </div>
      </div>

      {/* Main Content */}
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-4xl bg-white/95 backdrop-blur-sm rounded-3xl shadow-3xl p-12 space-y-10 border border-amber-200">
          {/* Success Icon */}
          <div className="flex flex-col items-center pb-8 border-b border-gray-200">
            <div className="bg-amber-100 p-8 rounded-full mb-6 animate-pulse">
              <CheckCircle className="w-28 h-28 text-amber-600" strokeWidth={2} />
            </div>
          ...
          {/* Credit Memo Details */}
          <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl p-8 space-y-5 border border-amber-200">
            <div className="grid grid-cols-2 gap-6">
              <div>
                <p className="text-sm text-gray-500 font-semibold mb-2">Credit Memo #</p>
                <p className="text-xl font-bold text-gray-900">{creditNumber}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500 font-semibold mb-2">Date & Time</p>
                <p className="text-xl font-bold text-gray-900">{returnDate}</p>
              </div>
              <div>
          ...
          <div className="space-y-5">
            ...
            <div className="max-h-80 overflow-y-auto space-y-3 pr-2">
              {cart.map((item: any) => (
                <div
                  key={item.id}
                  className="flex justify-between items-center p-5 bg-gradient-to-r from-gray-50 to-amber-50/30 rounded-2xl border border-gray-200 shadow-sm"
                >
                  <div>
                    <p className="font-bold text-gray-900 text-lg mb-1">{item.name}</p>
                    <p className="text-sm text-gray-600 font-medium">
                      {item.sku} × {Math.abs(item.quantity)}
                    </p>
                  </div>
          ...
          <div className="border-t-2 border-gray-200 pt-8">
            <div className="flex justify-between items-center mb-8 bg-amber-50 p-6 rounded-2xl border-2 border-amber-200">
              <span className="text-3xl font-bold text-gray-800">Credit Amount:</span>
          ...
          <div className="text-center text-base text-gray-600 pt-6 border-t border-gray-200 space-y-2">
            <p className="font-medium">
              Inventory has been incremented. Credit memo sent to accounting.
            </p>
